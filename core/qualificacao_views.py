...
from .models import Piloto, QualificacaoPiloto, Voo
...
def _perfil_contexto(piloto):
    voos = list(Voo.objects.select_related("drone").filter(piloto=piloto))
    total_minutos = sum(v.duracao_minutos for v in voos)
    por_drone = defaultdict(lambda: {"voos": 0, "minutos": 0})
    for voo in voos:
        por_drone[voo.drone.nome]["voos"] += 1
        por_drone[voo.drone.nome]["minutos"] += voo.duracao_minutos
    experiencia = [
        {"drone": nome, "voos": dados["voos"], "horas": round(dados["minutos"] / 60, 1)}
        for nome, dados in sorted(por_drone.items(), key=lambda item: -item[1]["minutos"])
    ]
    ...
    return {
        ...
        "total_voos_piloto": len(voos), "total_horas_piloto": round(total_minutos / 60, 1),
