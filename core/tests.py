...
class SelecaoPerfilAcessoTests(TestCase):
    ...
    def test_admin_pode_usar_o_mesmo_login_no_modo_coordenador(self):
        ...
        self.assertContains(painel, 'referrerPolicy: "origin"')
        self.assertContains(painel, 'typeof Chart !== "undefined"')
        self.assertContains(painel, 'window.addEventListener("resize"')
        ...
        self.assertContains(painel, "Inspeções da frota")
        ...
        self.assertEqual(risco.status_code, 200)
        self.assertTrue(risco.context["somente_leitura"])
        incidentes = self.client.get(reverse("incidentes"))
        self.assertEqual(incidentes.status_code, 200)
        self.assertContains(incidentes, "somente para consulta")
        self.assertNotContains(incidentes, "+ Registrar incidente")
        reservas = self.client.get(reverse("solicitacoes_voo"))
        self.assertContains(reservas, "Piloto de Outro Usuário")
        self.assertNotContains(reservas, "+ Reservar drone")
        self.assertRedirects(self.client.get(reverse("solicitacao_voo_nova")), reverse("solicitacoes_voo"))
        self.assertRedirects(self.client.get(reverse("solicitacao_voo_editar", args=[self.solicitacao_outro.pk])), reverse("solicitacoes_voo"))
        self.assertRedirects(self.client.post(reverse("solicitacao_voo_cancelar", args=[self.solicitacao_outro.pk])), reverse("solicitacoes_voo"))
        self.solicitacao_outro.refresh_from_db()
        self.assertEqual(self.solicitacao_outro.status, "aprovado")
        telemetria = self.client.get(reverse("telemetria_lista"))
        self.assertEqual(telemetria.status_code, 200)
        self.assertContains(telemetria, "somente para consulta")
        self.assertNotContains(telemetria, "+ Importar log")
        self.assertRedirects(self.client.get(reverse("telemetria_importar")), reverse("telemetria_lista"))
        self.assertContains(painel, "Trocar perfil")
        self.assertNotContains(painel, "Pilotos / Usuários")
        self.assertRedirects(self.client.get(reverse("pilotos")), reverse("dashboard"))
        voos = self.client.get(reverse("voos"))
        self.assertContains(voos, "Piloto de Outro Usuário")
        self.assertNotContains(voos, "+ Reservar Drone")
        planejamentos = self.client.get(reverse("planejamentos"))
        self.assertContains(planejamentos, "Área operacional de teste")
        self.assertNotContains(planejamentos, "Novo planejamento")
        detalhe = self.client.get(reverse("planejamento_detalhe", args=[self.planejamento_outro.pk]))
        self.assertEqual(detalhe.status_code, 200)
        self.assertNotContains(detalhe, ">Editar<", html=False)
        self.assertRedirects(self.client.get(reverse("planejamento_novo")), reverse("planejamentos"))
        self.assertEqual(self.client.get(reverse("planejamento_editar", args=[self.planejamento_outro.pk])).status_code, 404)
        agenda = self.client.get(reverse("minha_agenda"))
        self.assertContains(agenda, "Agenda Operacional")
        self.assertContains(agenda, "Piloto de Outro Usuário")
        calendario = self.client.get(reverse("calendario"))
        self.assertContains(calendario, "Piloto de Outro Usuário")
        self.assertNotContains(calendario, ">Editar<", html=False)
        self.assertRedirects(self.client.get(reverse("alocacao_nova")), reverse("calendario"))
        self.assertRedirects(self.client.get(reverse("alocacao_editar", args=[self.reserva_outro.pk])), reverse("calendario"))
        self.assertRedirects(self.client.post(reverse("alocacao_excluir", args=[self.reserva_outro.pk])), reverse("calendario"))
        self.assertTrue(Alocacao.objects.filter(pk=self.reserva_outro.pk).exists())
