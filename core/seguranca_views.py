...
def _pode_acessar_solicitacao(user, solicitacao):
    return usuario_tem_visao_global(user) or solicitacao.piloto.user_id == user.id
...
def avaliacao_risco(request, solicitacao_id):
    ...
    somente_leitura = bool(eh_admin or eh_coordenador or (avaliacao and avaliacao.status == "aprovada" and not pode_corrigir))
...
def incidente_novo(request):
    if usuario_e_coordenador(request.user):
        messages.info(request, "No perfil de coordenador, os incidentes ficam disponíveis somente para consulta.")
